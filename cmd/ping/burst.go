...
func main() {
	...
	timeout := flag.Float64("W", 1000000, "")
	probes := flag.Int("p", 1, "")
	gap := flag.Float64("g", 0, "")
	...
	if *deadline > 0 {
		ctx, cancel = context.WithTimeout(ctx, time.Duration(*deadline*float64(time.Microsecond)))
		defer cancel()
	}

	if *timeout <= 0 {
		*timeout = 2 * 1000000 // 2 seconds
	}
	...
	tick := time.NewTicker(time.Duration(*interval * float64(time.Microsecond)))
	if *gap != 0 && *probes != 1 {
		tick = time.NewTicker(time.Duration((*interval + ((*gap) * float64(*probes))) * float64(time.Microsecond)))
	}
	defer tick.Stop()

	// chanLength := math.MaxUint32
	// chanLength := math.MaxUint32
	if *count != 0 {
		// chanLength = *probes * *count
		// *count = chanLength
		*count = *probes * *count
	}
	welcome := fmt.Sprintf("Sending a total of %d packets\n", *count)
	// fmt.Printf("Sending a total of %d packets\n", *count)
        fmt.Printf(welcome)
	...
	var results = [][]string{}
	results = append(results, []string{"sendTime", "rcvTime", "src", "dst", "interval", "seq", "reqPayloadSize", "resPacketSize", "burst", "gap", "ttl", "rtt", "interface", "count"})
	...
	for *count == 0 || packetsSent < *count {
		...
		case t := <-tick.C:
			if *gap != 0 && *probes != 1 {
				fmt.Println("- Sending burst at", t)
			}

			ctx, cancel := context.WithTimeout(context.Background(), time.Duration(*timeout*float64(time.Microsecond)))

			if *gap != 0 && *probes != 1 {
				ctx, cancel = context.WithTimeout(context.Background(), time.Duration((*gap*float64(*probes))**timeout*float64(time.Microsecond)))

				time.Sleep(time.Duration(*interval * float64(time.Microsecond)))
				fmt.Printf(" -- [Burst of %d probes] Interval of %f microseconds\n", *probes, *interval)
			}

			defer cancel()

			// gapTick := time.NewTicker(time.Microsecond)
			// if *gap != 0 && *probes != 0 {
			// 	gapTick = time.NewTicker(time.Duration((*gap - 1 ) * float64(time.Microsecond)))
			// 	defer gapTick.Stop()
			// }
			...
			for i := 0; i < *probes; i++ {
				...
				if *gap != 0 && *probes != 1 {
					...
					go func(req ping.Request, seq int) {
						...
						if err != nil {
							fmt.Println("failed to ping:", err)
							saveErr(&req, now, seq, *size, *interval, *timeout, *probes, *gap, *iface, *count, &results)
							return
						}

						// resps <- resp
						resps = append(resps, resp)
						onRcv(resp, name, *size, *interval, *timeout, *probes, *gap, *iface, *count, &results)
					}(req, packetsSent)

					// fmt.Printf("  --- Packet No.%d\n", packetsSent)
					j++
					fmt.Printf("  --- [Gap of %f microseconds] Probe No. %d sent at %v\n", *gap, packetsSent, time.Now())
					if float64(j) != (*gap) {
						time.Sleep(time.Duration(*gap * float64(time.Microsecond)))
					}
		...
		// if *count == 0 && packetsSent == chanLength {
		// 	break
		// }
	}
...
finish:
	...
	onFin(packetsSent, resps, destination, results, *output)
}

func onRcv(res *ping.Response, name string, size int, interval float64, timeout float64, probes int, gap float64, iface string, count int, results *[][]string) {
	// Microsecond precision for the time
	layout := "2006-01-02T15:04:05.000000"

	if name != "" {
		fmt.Printf("%d bytes from %s (%s): icmp_seq=%d ttl=%v time=%v\n",
			res.TotalLength, name, res.Src.String(), res.Seq, res.TTL, res.RTT.Truncate(time.Microsecond))
	} else {
		fmt.Printf("%d bytes from %s: icmp_seq=%d ttl=%v time=%v\n",
			res.TotalLength, res.Src.String(), res.Seq, res.TTL, res.RTT.Truncate(time.Microsecond))
	}

	sentDateTime := res.Req.SentAt.Format(layout)
	rcvDateTime := res.RcvdAt.Format(layout)
	response := []string{
		fmt.Sprintf("%v", sentDateTime),
		fmt.Sprintf("%v", rcvDateTime),
		res.Req.Src.String(),
		res.Src.String(),
		fmt.Sprintf("%.6f", interval),
		fmt.Sprintf("%d", res.Seq),
		fmt.Sprintf("%d", size),
		fmt.Sprintf("%d", res.TotalLength),
		fmt.Sprintf("%d", probes),
		fmt.Sprintf("%f", gap),
		fmt.Sprintf("%v", res.TTL),
		fmt.Sprintf("%v", res.RTT.Truncate(time.Microsecond)),
		iface,
		fmt.Sprintf("%d", count),
	}
	...
	createResult(results, response)

	// return

}

func onFin(packetsSent int, resps []*ping.Response, destination string, results [][]string, output string) {
	if packetsSent == 0 {
	...
	if output != "" {
		if err := csvExport(results, output); err != nil {
			fmt.Println("failed to export to csv:", err)
			log.Fatal(err)
		}
	}
...
func createResult(collector *[][]string, data []string) {
	*collector = append(*collector, data)
}

func saveErr(req *ping.Request, now time.Time, seq int, size int, interval float64, timeout float64, probes int, gap float64, iface string, count int, results *[][]string) {
	// Microsecond precision for the time
	layout := "2006-01-02T15:04:05.000000"

	sentDateTime := now.Format(layout)
	rcvDateTime := ""
	response := []string{
		fmt.Sprintf("%v", sentDateTime),
		fmt.Sprintf("%v", rcvDateTime),
...
func csvExport(data [][]string, output string) error {
...
OPTIONS
       ...
       -p probe
           Number of probes ICMP to send. The default is 1, like a normal ping.
           Other value enable multiple probes behavior, like in traceroute.
